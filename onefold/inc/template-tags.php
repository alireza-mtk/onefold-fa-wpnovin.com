	...
	function onefold_entry_footer() {
		...
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			...
			if ( true === $show_meta_comment ) {
				...
				comments_popup_link( esc_html__( 'ارسال دیدگاه', 'onefold' ), esc_html__( '1 دیدگاه', 'onefold' ), esc_html__( '% دیدگاه', 'onefold' ) );
				echo '</span>';
			}
		}
		...
		edit_post_link( esc_html__( 'ویرایش', 'onefold' ), '<span class="edit-link">', '</span>' );
	}
endif;
